...
import { useMainDashboard, useInvalidate } from '../../queries.js';
...
export default function MainOrganizations() {
  const { token } = useAuth();
  ...
  const toggle = async (p) => {
    ...
    try {
      await api.mainSetPartnerStatus(token, p.id, next);
          ...
          <div className="card-body gap-4">
            <div className="flex flex-wrap items-center gap-3">
              <label className="input input-bordered input-sm max-w-xs flex items-center gap-2">
                ...
                <input className="grow" placeholder="Поиск по названию / домену…" value={q} onChange={(e) => setQ(e.target.value)} />
              </label>
              <div className="join">
                {FILTERS.map((f) => (
                ...
                <tbody>
                  {rows.length === 0 ? (
                    <tr><td colSpan={8} className="text-center py-10">
                      ...
                      <div className="opacity-50 text-sm">{q || statusFilter !== 'all' ? 'Ничего не найдено' : 'Партнёров нет'}</div>
                    </td></tr>
                  ) : (
                    rows.map((p) => {
                        ...
                        <tr key={p.id} className="hover">
                          <td>
                            <div className="flex items-center gap-2.5">
                              ...
                              <Link to={`/main/organizations/${p.id}`} className="font-medium hover:text-primary transition-colors">{p.name}</Link>
                            </div>
                          </td>
                          ...
                          <td>
                            <span className={`badge badge-sm gap-1 ${s.cls}`}>
                              {StatusIcon && <StatusIcon size={11} />}{s.label}
                            </span>
                          </td>
                          <td className="whitespace-nowrap text-sm text-base-content/60">{dateShort(p.createdAt)}</td>
                          <td>
                            <div className="flex items-center gap-1.5 justify-end">
                              <Link to={`/main/organizations/${p.id}`} className="btn btn-xs btn-ghost"><ArrowRight size={13} /></Link>
                              <button className={`btn btn-xs ${p.status === 'frozen' ? 'btn-success' : 'btn-outline btn-error'}`} onClick={() => toggle(p)}>
