...
export default function MainBilling() {
  ...
  const { data: pricing, isLoading: pLoading } = useMainPricing();
  const { data: dash } = useMainDashboard();
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        ...
        subtitle={`Цена по факту: первый филиал + доп. филиалы + ученики (в ${cur}). Счёт растёт автоматически.`}
      />

      {error && <div className="alert alert-error text-sm"><span>{error}</span></div>}

      {pLoading ? (
        <><SkeletonKpis count={3} /><SkeletonTable rows={4} cols={4} /></>
      ) : (
        <>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            <Kpi Icon={Wallet} tint={{ bg: '#E6F4D7', fg: '#3F6212' }} title="Общий счёт / мес" value={fmt(partners.reduce((s, p) => s + p.monthlyBill, 0))} unit={cur} />
            <Kpi Icon={Building2} tint={{ bg: '#E0F2FE', fg: '#075985' }} title="Партнёров на биллинге" value={fmt(partners.length)} unit="учебных центров" />
            <Kpi Icon={Users} tint={{ bg: '#EDE9FE', fg: '#5B21B6' }} title="Средний счёт" value={fmt(partners.length ? Math.round(partners.reduce((s, p) => s + p.monthlyBill, 0) / partners.length) : 0)} unit={cur} />
          </div>
                ...
                {FIELDS.map((f) => (
                  <label key={f.key} className="form-control">
                    <span className="label-text mb-1 flex items-center gap-1.5">
                      <f.Icon size={14} className="text-base-content/40" />{f.label}
                    </span>
                    <div className="join">
