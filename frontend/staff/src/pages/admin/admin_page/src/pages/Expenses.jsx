...
import {
  HiOutlineMagnifyingGlass, HiOutlinePlus, HiOutlineTrash, HiOutlineCurrencyDollar,
  HiOutlineCalendarDays, HiOutlineChartBarSquare, HiOutlineArrowPath,
  HiOutlineEllipsisVertical, HiOutlineEye, HiOutlinePencilSquare,
  HiOutlineXMark,
  HiOutlineBanknotes,
  HiOutlineReceiptPercent, HiOutlineChevronDown, HiOutlineExclamationTriangle,
  HiOutlineDocumentArrowDown, HiOutlineAdjustmentsHorizontal,
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        ...
        >
          ...
          >
            <span className="w-5 h-5 flex items-center justify-center"><HiOutlineEye className="w-4 h-4" /></span>
            Ko'rish
          </button>
          <button
            ...
            className="w-full flex items-center gap-2.5 px-4 py-2.5 text-[12px] font-semibold text-[var(--text-secondary)] hover:text-[var(--text)] hover:bg-[var(--surface-hover)] transition-colors"
          >
            <span className="w-5 h-5 flex items-center justify-center"><HiOutlinePencilSquare className="w-4 h-4" /></span>
            Tahrirlash
          </button>
          ...
          >
            <span className="w-5 h-5 flex items-center justify-center"><HiOutlineTrash className="w-4 h-4" /></span>
            O'chirish
          </button>
        </div>
      )}
    ...
    <div className="space-y-6 page-enter pb-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          <Button variant="ghost" size="sm" onClick={handleExport} disabled={exporting || filtered.length === 0}>
      ...
      <div className="glass-strong rounded-[20px] overflow-hidden card-hover-premium">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3 p-4">
          ...
          <div className="relative flex-1 w-full sm:max-w-xs">
            <HiOutlineMagnifyingGlass className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[var(--text-muted)] pointer-events-none" />
            <input
        ...
        <div className="min-w-0">
          {loading && expenses.length === 0 ? (
            ...
            <div className="glass-strong rounded-[20px]">
              <EmptyState
                title={search || hasActiveFilters ? "Natija topilmadi" : "Hozircha xarajatlar yo'q"}
                description={search || hasActiveFilters ? "Boshqa qidiruv yoki filtr sozlamalarini sinab ko'ring" : "Birinchi xarajatni qo'shish orqali boshlang"}
                action={search || hasActiveFilters ? undefined : { label: "Xarajat qo'shish", onClick: openModal }}
              >
                      ...
                      >
                        ...
                        <td className="px-5 py-4 text-[var(--text-secondary)] tabular-nums whitespace-nowrap text-[12px]">
                          <span className="flex items-center gap-1.5">
                            <HiOutlineCalendarDays className="w-3 h-3 text-[var(--text-muted)]" />
      ...
      {/* ═══ View Detail Modal ═══ */}
      <Modal open={viewModalOpen} title="Xarajat tafsilotlari" onClose={() => { setViewModalOpen(false); setViewTarget(null); }}>
        {viewTarget && (
          <div className="space-y-5">
            ...
            <div className="flex justify-end gap-2.5 pt-3 border-t border-[var(--border)]">
              <Button variant="ghost" size="sm" onClick={() => { setViewModalOpen(false); setViewTarget(null); }}>
                Yopish
              </Button>
              <Button variant="primary" size="sm" onClick={() => { setViewModalOpen(false); setViewTarget(null); openEditModal(viewTarget); }}>
      ...
      {/* ═══ Add/Edit Modal ═══ */}
      <Modal open={modalOpen} onClose={() => { if (!saving) setModalOpen(false); }} title="Xarajat qo'shish">
        <div className="space-y-5">
          <div>
          ...
          <div className="flex justify-end gap-2.5 pt-2">
            ...
            <Button variant="primary" size="sm" onClick={handleSave} disabled={saving || !formData.amount}>
              {saving ? (
                <span className="flex items-center gap-1.5">
        ...
        <div className="space-y-5">
          <div className="flex items-start gap-4">
            <div className="w-11 h-11 rounded-[14px] bg-[rgba(232,84,62,0.12)] flex items-center justify-center shrink-0">
              <HiOutlineExclamationTriangle className="w-5 h-5 text-[var(--danger)]" />
            </div>
...
function SelectFilter({ value, onChange, options, placeholder }) {
  return (
    <div className="relative w-full sm:w-auto">
      ...
      <HiOutlineChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[var(--text-muted)] pointer-events-none" />
    </div>
  );
}
