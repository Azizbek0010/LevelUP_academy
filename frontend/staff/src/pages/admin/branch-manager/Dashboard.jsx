...
import { BRANCH, MONTHS, MONTHLY_SUMMARY, PAYMENTS } from './_data.js';

export default function BranchManagerDashboard() {
  const cur = MONTHLY_SUMMARY[MONTHLY_SUMMARY.length - 1];
  const prev = MONTHLY_SUMMARY[MONTHLY_SUMMARY.length - 2] || cur;
  ...
  return (
    <div className="space-y-6 pb-8 animate-page-enter">
      <PageHeader
        ...
        subtitle={`${BRANCH.name} · filialning umumiy holati`}
      />
        ...
        <Panel title="Filial" icon={Building2} bodyClass="p-4">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              <h3 className="font-bold text-[15px] text-base-content truncate">{BRANCH.name}</h3>
          ...
          <dl className="grid grid-cols-3 gap-3 mt-5 pt-4 border-t border-base-200">
            <div>
              ...
              <dd className="font-bold tabular-nums">{fmt(BRANCH.stats.students)}</dd>
            </div>
            <div>
              <dt className="text-[11px] text-base-content/45">Guruhlar</dt>
              <dd className="font-bold tabular-nums">{fmt(BRANCH.stats.groups)}</dd>
            </div>
            <div>
              <dt className="text-[11px] text-base-content/45">Xodimlar</dt>
              <dd className="font-bold tabular-nums">{fmt(BRANCH.stats.staff)}</dd>
            </div>
          </dl>
        </Panel>
