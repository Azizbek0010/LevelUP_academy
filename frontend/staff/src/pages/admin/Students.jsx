            ...
            <table className="table-premium">
              <thead>
                <tr>
                  <th>Студент</th>
                  <th>Код</th>
                  <th className="tabular-nums">Телефон</th>
                  <th>Группы</th>
              ...
              <tbody>
                {filteredRows.map(s => (
                  <tr key={s.id} className="cursor-pointer" onClick={() => navigate(`/students/${s.id}`)}>
                    ...
                    <td className="font-mono tabular-nums text-[var(--text-secondary)]">{s.login_code || s.loginCode || '—'}</td>
                    <td className="tabular-nums text-[var(--text-muted)]">{s.phone || '—'}</td>
                    <td>
                    ...
                    <td className="text-right">
                      {s.coins != null && s.coins > 0 ? (
                        <span className="inline-flex items-center gap-1 text-[var(--green)] font-semibold tabular-nums">
                    ...
                    <td>
                      <StatusBadge status={s.status === 'frozen' ? 'frozen' : 'active'} />
                    </td>
                    <td>
                      <div className="flex items-center gap-1" onClick={(e) => e.stopPropagation()}>
                        <button className="w-7 h-7 rounded-[8px] flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--surface-hover)] hover:text-[var(--text)] transition-all" title="Сбросить пароль" onClick={() => regen(s)}>
                          <KeyRound size={12} />
                        </button>
                        <button className="w-7 h-7 rounded-[8px] flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--surface-hover)] hover:text-[var(--text)] transition-all"
                          title={s.status === 'frozen' ? 'Разморозить' : 'Заморозить'} onClick={() => toggleFreeze(s)}>
                          {s.status === 'frozen' ? <Sun size={12} /> : <Snowflake size={12} />}
                        </button>
                        <button className="w-7 h-7 rounded-[8px] flex items-center justify-center text-[var(--text-muted)] hover:bg-[var(--danger-light)] hover:text-[var(--danger)] transition-all" title="Удалить" onClick={() => del(s)}>
                          <Trash2 size={12} />
                        </button>
                      </div>
                    </td>
