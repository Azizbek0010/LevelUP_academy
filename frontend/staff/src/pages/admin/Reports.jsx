...
export default function AdminReports() {
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      {/* ═══ Table ═══ */}
      <div className="table-container animate-fade-in stagger-5">
        <div className="table-header">
          <h3>Детали по группам</h3>
          <span className="text-[11px] text-[var(--text-muted)] tabular-nums">{byGroup.length} групп</span>
        </div>
        <div className="overflow-x-auto">
          <table className="table-premium">
            ...
            <tbody>
              {byGroup.length === 0 && (
                <tr><td colSpan={4} className="text-center text-[var(--text-muted)] py-10 text-[13px]">Нет данных</td></tr>
              )}
              {byGroup.map((g, i) => {
                ...
                return (
                  <tr key={g.id || g.groupId || i}>
                    ...
                    <td className="text-right tabular-nums font-semibold" style={{ color: 'var(--success)' }}>{money(g.revenue)}</td>
                    <td className="text-right tabular-nums font-semibold" style={{ color: debt > 0 ? 'var(--danger)' : 'var(--text-muted)' }}>{money(debt)}</td>
                  </tr>
                );
              })}
            </tbody>
