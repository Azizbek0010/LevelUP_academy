...
function MentorCard({ m, onEdit, onFreeze, onDelete }) {
  ...
  const status = STATUS_CLASSES[m.status] || STATUS_CLASSES.active;
        ...
        <dialog className="modal modal-open">
          <div className="modal-box card bg-base-100 border border-base-300">
            <div className="flex justify-between items-start mb-4">
              <h3 className="font-bold text-lg">{form.id ? 'Изменить ментора' : 'Новый ментор'}</h3>
              <button className="btn btn-ghost btn-sm btn-square -mt-1 -mr-1" onClick={() => setForm(null)}><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg></button>
            </div>
        ...
        <dialog className="modal modal-open">
          <div className="modal-box card bg-base-100 border border-base-300 max-w-sm">
            <div className="flex justify-between items-start mb-1">
              <h3 className="font-bold text-lg flex items-center gap-2"><KeyRound size={18} className="text-primary" /> Ментор создан</h3>
              <button className="btn btn-ghost btn-sm btn-square -mt-1 -mr-1" onClick={() => setCreds(null)}><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 6 6 18"/><path d="m6 6 12 12"/></svg></button>
            </div>
