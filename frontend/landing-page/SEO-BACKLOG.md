...
- [ ] **Yandex: включить email-уведомления** — ⚠️ **делается не в Вебмастере.** Дропдаун
      «Адрес для почтовых уведомлений» предлагает только «Другой адрес», и он редиректит в
      **Яндекс ID → Контакты** (`id.yandex.ru/personal/contacts`): у аккаунта нет ни одной
      подтверждённой почты, а подтверждение идёт письмом. Шаги: добавить и подтвердить адрес
      в Яндекс ID → вернуться в Настройки Вебмастера → выбрать его → «Сохранить». Пока стоит
      «Не получать» — уведомления «Ошибки» и «Изменение регионов сайта» никуда не уходят.
      Только шаг в Яндекс ID делает владелец аккаунта.
