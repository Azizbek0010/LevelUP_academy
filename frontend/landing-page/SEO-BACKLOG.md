...
- [ ] **GSC + IndexNow: 2 URL «CRM вместо Excel»** (`/landing/crm-vs-excel`, `/uz/...`) —
      **только после деплоя в прод**, иначе пинг уйдёт на 404. `npm run indexnow` берёт URL
      из sitemap, там они уже есть.
