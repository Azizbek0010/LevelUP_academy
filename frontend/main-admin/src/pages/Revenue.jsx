    ...
    <div className="flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex justify-between items-center text-sm mb-1 gap-2">
          <Link to={`/organizations/${id}`} className="truncate font-medium hover:text-primary transition-colors">
            {name}
          </Link>
        ...
        <div className="h-2.5 rounded-full bg-base-200 overflow-hidden">
          <div
            ...
            style={{ width: `${pct}%`, background: '#C6FF34' }}
          />
        </div>
      </div>
    </div>
...
export default function Revenue() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      partners, cur, total, avg, sorted, maxBill,
      active, trial, frozen,
              ...
              <div className="card bg-base-100">
                <div className="card-body p-4">
                  <p className="text-xs text-base-content/40 leading-relaxed">
                    График тренда дохода по месяцам появится после накопления истории платежей (модуль K-ADMIN).
                  </p>
                </div>
              </div>
            </div>
                        ...
                        <tr key={p.id} className="hover">
                          ...
                          <td>
                            <div className="flex items-center gap-2">
                              ...
                              <Link to={`/organizations/${p.id}`} className="font-medium hover:text-primary">{p.name}</Link>
                            </div>
                          </td>
