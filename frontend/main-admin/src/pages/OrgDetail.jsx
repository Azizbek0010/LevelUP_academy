        ...
        <div className="space-y-2.5">
          ...
          {Math.abs(calc - (partner.monthlyBill || 0)) > 1 && (
            <p className="text-xs text-base-content/40">
              * По текущим тарифам: {fmt(calc)}. Фактический счёт: {fmt(partner.monthlyBill)} {cur}.
            </p>
          )}
        </div>
      </div>
...
export default function OrgDetail() {
  ...
  return (
    <div className="space-y-5">
      <button className="btn btn-ghost btn-sm gap-2 -ml-2" onClick={() => navigate('/organizations')}>
            ...
            <div className="card-body flex-row flex-wrap items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-4 text-sm text-base-content/55">
                  {partner.domain && (
                    <span className="flex items-center gap-1.5"><Globe size={13} />{partner.domain}</span>
                  )}
                  <span className="flex items-center gap-1.5">
                    <Calendar size={13} />Зарегистрирован {dateShort(partner.createdAt)}
                  </span>
