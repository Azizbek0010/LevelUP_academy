...
import { User, Shield, CreditCard, Building2, GraduationCap, Wallet, Info, ExternalLink } from 'lucide-react';
          ...
          {!t ? (
            ...
            <>
              ...
              <Row
                label="Доход / мес"
                value={
                  <span className="text-primary font-bold">{fmt(t.ourMonthlyIncome)} {cur}</span>
                }
              />
              <div className="mt-3">
                <div className="grid grid-cols-3 gap-2">
                  {(data?.partners || []).reduce((acc, p) => {
                    acc[p.status] = (acc[p.status] || 0) + 1;
                    return acc;
                  }, {}) && Object.entries(
                    (data?.partners || []).reduce((acc, p) => { acc[p.status] = (acc[p.status] || 0) + 1; return acc; }, {})
                  ).map(([status, count]) => {
        ...
        <Section title="Текущие тарифы" Icon={CreditCard}>
          {!pricing ? (
            ...
            <>
              ...
              <div className="mt-4">
                <Link to="/billing" className="btn btn-sm btn-outline btn-primary gap-1.5 w-full">
      ...
      <div className="card bg-base-100">
        <div className="card-body">
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            <Link to="/leads" className="flex items-center gap-3 p-3 rounded-xl border border-base-300 hover:border-primary hover:text-primary transition-colors group">
            ...
            <Link to="/organizations" className="flex items-center gap-3 p-3 rounded-xl border border-base-300 hover:border-primary hover:text-primary transition-colors group">
            ...
            <Link to="/revenue" className="flex items-center gap-3 p-3 rounded-xl border border-base-300 hover:border-primary hover:text-primary transition-colors group">
