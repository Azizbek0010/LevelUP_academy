...
> что тут стоит как `[ ]`, давно написано (Main Admin, SEO, Admin, платежи,
...
| **SEO** | партнёр = учебный центр (напр. Mars IT school) | своя организация: свои филиалы, свой доход |
...
Поток онбординга: центр оставляет **заявку на лендинге** → она падает в **Main Admin** → Main Admin создаёт **SEO** (заводит организацию) → SEO сам добавляет филиалы и админов → Admin добавляет студентов/родителей.
...
| **SEO** — организация партнёра, CRUD филиалов, CRUD админов | **Student** — home, магазин, тесты, ДЗ, видео, лидерборд |
...
- [x] **A3. Роль `main_admin`** в enum `user_role`, выше `seo`. *(enum: main_admin, seo, admin, mentor, parent, student)*
- [ ] **A4. SEO перескоупить** — видит только свою организацию (`organization_id`), не все филиалы глобально. ⚠️ затрагивает зону Abdulaziz.
...
- [x] Онбординг партнёра: `POST /api/main/partners` — создать `organization` (план+домен) + `seo` (temp-пароль). Проверено вживую. *(модуль `modules/main`, миграция `1752000000000_org-billing`)*
...
### K-SUPER — SEO (организация партнёра) ✅ закрыто (сверено 2026-07-26)
