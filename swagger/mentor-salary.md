...
`groupRevenue = monthlyPrice × activeStudents` per group the mentor teaches during the month. Access: the mentor may only view their own suggestion; admin only for mentors in their own branch (404 if a foreign branch, to avoid disclosure); superadmin/main_admin unrestricted.
